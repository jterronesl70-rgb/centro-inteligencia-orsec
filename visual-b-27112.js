(function(){
 const TEMPLATE_B='ORSEC_VISUAL_B_REFERENCIA.png?v=27112';
 ...
 async function generar(){
   ...
   c.fillStyle=C.navy2;c.fillRect(1258,85,260,22);tx(c,'PERÍODO DE ANÁLISIS',1388,96,13,true,C.white,'center');
   rr(c,1258,107,260,54,0,'rgba(255,255,255,.97)',null);
   fit(c,`Año: ${y}  |  ${tipo==='MENSUAL'?'Mensual':'Acumulado'}: ${label}`,1272,125,230,12,true,C.text);
   fit(c,`Provincia: ${prov==='TOTAL REGION'?'Todas':nice(prov)}  |  Delito: ${del==='TODOS'?'Todos':nice(del)}`,1272,147,232,11,true,C.text);
   ...
   c.fillStyle=C.white;c.fillRect(270,665,757,267);tx(c,'Comparativo por delito',286,682,17,true,C.text);fit(c,`(${py} vs ${y} – Total de casos)`,286,702,320,13,false,C.text);tx(c,`■ ${py}`,620,692,13,true,C.blue2);tx(c,`■ ${y}`,710,692,13,true,C.navy2);
   ...
   chartD.forEach((d,i)=>{const a=N(dp[d]),b=N(dc[d]),v=pct(b,a),mid=cx+gw*i+gw/2,bw=30,ha=ch*a/max,hb=ch*b/max;c.fillStyle=C.blue2;c.fillRect(mid-bw-3,cy+ch-ha,bw,ha);c.fillStyle=C.navy2;c.fillRect(mid+3,cy+ch-hb,bw,hb);tx(c,F(a),mid-bw/2-3,cy+ch-ha-9,10,true,C.text,'center');tx(c,F(b),mid+bw/2+3,cy+ch-hb-9,10,true,C.text,'center');const nm=d==='ROBO DE VEHICULOS'?['Robo de','vehículos']:[nice(d).charAt(0)+nice(d).slice(1).toLowerCase()];nm.forEach((q,j)=>tx(c,q,mid,cy+ch+17+j*13,10,true,C.text,'center'));tx(c,(v!=null&&v<=0?'↓ ':'↑ ')+P(v),mid,cy+ch+52,13,true,v!=null&&v<=0?C.green:C.red,'center')});
   ...
   const shares=[['EXTORSION',C.orange],['HURTOS','#1597e5'],['ROBOS','#6d36c7'],['ROBO DE VEHICULOS',C.red],['HOMICIDIOS','#18a73d'],['DETONACIONES','#7448d6'],['VIOLACION SEXUAL','#9c73e6']];const fixedSum=shares.reduce((a,[d])=>a+N(dc[d]),0),other=Math.max(0,totalCur-fixedSum),seg=[...shares.map(([d,col])=>[nice(d),N(dc[d]),col]),['Otros',other,C.gray]],dtotal=Math.max(1,totalCur),centerX=1172,centerY=810,R=94,r=48;let ang=-Math.PI/2;
   ...
   seg.forEach(([name,val,col],i)=>{const yy=742+i*22;c.fillStyle=col;c.fillRect(1295,yy-7,11,11);fit(c,`${name} ${(val/dtotal*100).toFixed(1)}%`,1313,yy,190,10,i<7,C.text)});

   // Pie institucional
   c.fillStyle=C.white;c.fillRect(15,954,1000,49);tx(c,'Fuente: Unidad de Estadística de la Región Policial de La Libertad.',23,968,11,true,C.text);tx(c,'Elaboración: Observatorio Regional de Seguridad Ciudadana (ORSEC) – La Libertad.',23,986,10,false,C.text);tx(c,'“Cada dato tiene un territorio. Cada territorio tiene personas. Cada persona merece vivir segura.”',770,982,10,false,C.text,'center');tx(c,'V27.11.2 · VISUAL B DINÁMICA · EXPERIMENTAL',1510,1002,9,true,'#6d8192','right');
   return can.toDataURL('image/png');
 }
 window.orsecGenerarVisualB=generar;
 window.ORSEC_EXPERIMENTAL_BUILD='V27.11.2-VISUAL-B-DISENO-REAL-DINAMICO';
})();
